...
| Sparse / conditioning paths | Yes | Yes | Yes | sparse Cholesky measured: 22-89× factorization speedup over dense LU, 0 perturbations, fill 2.6-7.0× (grows with mesh size); sparse reuse is partly done in modal/buckling/harmonic/reduction, modal 3D now has a sparse eigensolver path; next blockers are deeper sparse eigensolver integration, runtime measurement on the newly sparse workflows, and ordering/fill policy |
...
- performance and scale maturity
  sparse Cholesky runtime gains are now measured (22-89× factorization speedup, 22× end-to-end, 0 perturbations); sparse reuse is partly done into modal/buckling/harmonic/reduction, modal 3D now has a sparse eigensolver path, and the remaining work is deeper sparse eigensolver integration, runtime measurement on the newly sparse workflows, ordering-policy selection, and eigensolver cleanup
- verification depth
...
|----------|-------|---------|
| 1 | Performance and scale engineering | Sparse runtime gains measured (22-89× factorization, 22× end-to-end, 0 perturbations). Modal 3D now has a sparse eigensolver path (`11.8×` speedup at 20×20 MITC4). Next: deepen sparse eigensolver integration, investigate AMD vs RCM for ordering policy, and clean up eigensolver debt. |
...
|----------|-------|----------------|
| 1 | Performance / scale engineering | Sparse runtime gains measured (22-89× factorization, 22× end-to-end). Sparse reuse is partly done in modal/buckling/harmonic/reduction, modal 3D already has a sparse eigensolver path, and the next steps are deeper sparse eigensolver integration, ordering-policy selection, and eigensolver robustness. |
