...
fn validation_biaxial_plate_buckling_interaction() {
    ...
    helpers::assert_close(interaction, 1.0, 1e-10, "equal biaxial interaction = 1.0");

    // Under uniaxial only: full sigma_cr is available
    let interaction_uniax: f64 = sigma_cr / sigma_cr + 0.0;
    helpers::assert_close(interaction_uniax, 1.0, 1e-10, "uniaxial at limit");
    ...
    helpers::assert_close(ratio_cr, 2.56, 1e-10, "cr_x/cr_y ratio for a/b=2");
    ...
    helpers::assert_close(check, 1.0, 1e-10, "rectangular plate equal biaxial interaction");
...
fn validation_cylindrical_shell_torsion_buckling() {
    ...
    helpers::assert_close(z, expected_z, 1e-10, "Batdorf parameter Z");
...
fn validation_spherical_shell_external_pressure() {
    ...
    helpers::assert_close(p_cr, expected_p, 1e-4, "spherical shell classical pressure");
    ...
    helpers::assert_close(ratio_p, expected_ratio, 1e-10, "pressure scales with (t/R)^2");
    ...
    helpers::assert_close(p_design / p_cr, 0.18, 1e-10, "knockdown factor");
...
fn validation_orthotropic_plate_buckling() {
    ...
    helpers::assert_close(nu_yx_orth, 0.35 * 4000.0 / 12000.0, 1e-10, "reciprocal nu_yx");
    ...
    helpers::assert_close(d_1, d_1_check, 1e-8, "reciprocity of D_1");
...
fn validation_ring_stiffened_cylinder_pressure() {
    ...
    helpers::assert_close(p_cr_segment, p_cr_unstiffened, 1e-10, "long cyl p_cr independent of L");
    ...
    helpers::assert_close(ratio_heavy, i_ratio, 1e-10, "stiffener contribution scales with I_s");
}
...
fn validation_conical_shell_axial_buckling() {
    ...
    helpers::assert_close(r_mid, 1000.0, 1e-10, "mid radius");
    ...
    helpers::assert_close(sigma_cr, sigma_cr_simplified, 1e-10, "simplified cone formula");
...
fn validation_plate_winkler_foundation_buckling() {
    ...
    helpers::assert_close(ab_ratio, 2.0, 1e-10, "aspect ratio a/b");
