...
fn aluminum_thermal_restraint_force() {
    ...
    helpers::assert_close(f_al, f_al_expected, 0.001, "Aluminum thermal force");

    // Steel: 12e-6 * 50 * 200000 * 3000 / 1000 = 360.0 kN
    let f_steel_expected: f64 = 12.0e-6 * 50.0 * 200_000.0 * 3000.0 / 1000.0;
    helpers::assert_close(f_steel, f_steel_expected, 0.001, "Steel thermal force");
    ...
    helpers::assert_close(ratio, ratio_expected, 0.001, "Al/steel thermal force ratio");
...
fn aluminum_truss_panel_deflection() {
    ...
    helpers::assert_close(defl_ratio, e_ratio, 0.02, "Truss deflection ratio Al/Steel");
    ...
    helpers::assert_close(r1_al.ry, r1_st.ry, 0.01, "Reaction R1y same for Al and Steel");
}
...
fn aluminum_plate_local_buckling() {
    ...
    helpers::assert_close(sigma_cr_int, sigma_cr_int_expected, 0.001, "Internal plate sigma_cr");
    ...
    helpers::assert_close(k_ratio, 0.10625, 0.001, "k_out/k_int ratio");
...
fn aluminum_welded_beam_haz_capacity() {
    ...
    helpers::assert_close(mid_d.uy.abs(), delta_exact, 0.02, "Al beam midspan deflection");
    ...
    helpers::assert_close(rho_haz, 0.456, 0.01, "HAZ strength reduction factor");

    // Verify capacity ratio equals inverse of HAZ factor
    let capacity_ratio: f64 = m_haz / m_parent;
    helpers::assert_close(capacity_ratio, rho_haz, 0.001, "Moment capacity ratio = rho_haz");
}
...
fn aluminum_alloy_property_comparison() {
    ...
    helpers::assert_close(ratio_5083, 1.419, 0.01, "5083-H116 fu/fy ratio");
    helpers::assert_close(ratio_6061, 1.203, 0.01, "6061-T6 fu/fy ratio");
    helpers::assert_close(ratio_6082, 1.137, 0.01, "6082-T6 fu/fy ratio");
...
fn aluminum_continuous_beam_two_span() {
    ...
    helpers::assert_close(r_a.ry, r_end_expected, 0.02, "R_A = 3qL/8");
    helpers::assert_close(r_b.ry, r_mid_expected, 0.02, "R_B = 10qL/8");
    helpers::assert_close(r_c.ry, r_end_expected, 0.02, "R_C = 3qL/8");

    // Equilibrium check: sum of reactions = total load
    let total_load: f64 = q_abs * 2.0 * l;
    let sum_ry: f64 = results.reactions.iter().map(|r| r.ry).sum();
    helpers::assert_close(sum_ry, total_load, 0.01, "Equilibrium: sum Ry = q*2L");
    ...
    helpers::assert_close(ef_at_mid.m_end.abs(), m_b_expected, 0.02, "M_B = qL^2/8");
}
...
fn aluminum_portal_frame_lateral() {
    ...
    helpers::assert_close(sum_rx, -lateral_h, 0.02, "Sum Rx = -H");

    // Total vertical reactions must be zero (no vertical load)
    let sum_ry: f64 = results.reactions.iter().map(|r| r.ry).sum();
    helpers::assert_close(sum_ry, 0.0, 0.05, "Sum Ry = 0");
    ...
    helpers::assert_close(r1.rx.abs(), v_col_expected, 0.15, "Left column shear ~ H/2");
    helpers::assert_close(r4.rx.abs(), v_col_expected, 0.15, "Right column shear ~ H/2");
    ...
    helpers::assert_close(overturning, resisting, 0.05, "Moment equilibrium about base");
}
...
fn aluminum_effective_width_method() {
    ...
    helpers::assert_close(rho_1, 1.0, 0.001, "Compact plate: full effective width");
    ...
    helpers::assert_close(rho_2, 0.667, 0.05, "Slender plate effective width ratio");
    ...
    helpers::assert_close(capacity_reduction, rho_2, 0.001, "Area reduction = rho");
}
