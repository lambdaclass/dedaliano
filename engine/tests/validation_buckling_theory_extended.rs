...
fn validation_southwell_plot_pcr_estimation() {
    ...
    helpers::assert_close(pcr_estimated, pcr, 0.001, "Southwell Pcr estimation");
    helpers::assert_close(e0_estimated, e0, 0.001, "Southwell e0 estimation");
}
...
fn validation_perry_robertson_imperfection() {
    ...
    helpers::assert_close(sigma_e, expected_sigma_e, 1e-10, "Euler stress at lambda=80");
    ...
    helpers::assert_close(sigma_fail, expected_fail, 1e-10, "Perry-Robertson at lambda=80");
}
...
fn validation_secant_formula_eccentric_compression() {
    ...
    helpers::assert_close(r, 0.1, 1e-10, "radius of gyration");
    ...
    helpers::assert_close(arg, expected_arg, 1e-10, "secant arg at P=500");

    let expected_sec = 1.0 / expected_arg.cos();
    let expected_sigma = (p / a) * (1.0 + (0.05 * 0.15 / 0.01) * expected_sec);
    helpers::assert_close(sigma_max, expected_sigma, 1e-10, "secant sigma_max at P=500");
...
fn validation_rayleigh_quotient_energy_method() {
    ...
    helpers::assert_close(pcr_sine, pcr_exact, 0.001,
    ...
    helpers::assert_close(pcr_para, pcr_para_exact, 0.001,
    ...
    helpers::assert_close(ratio, expected_ratio, 0.001,
...
fn validation_stepped_column_piecewise_ei() {
    ...
    helpers::assert_close(pcr_rr_numerical, pcr_rr_analytic, 0.005,
    ...
    helpers::assert_close(pcr_rr_analytic, pcr_midpoint, 1e-10,
        "RR Pcr equals midpoint of uniform bounds");

    // Verify specific values
    helpers::assert_close(ei1, 40_000.0, 1e-10, "EI1 = 40000 kN*m^2");
    helpers::assert_close(ei2, 20_000.0, 1e-10, "EI2 = 20000 kN*m^2");
    helpers::assert_close(ei_avg, 30_000.0, 1e-10, "EI_avg = 30000 kN*m^2");

    let expected_pcr_rr = PI * PI * 30_000.0 / 64.0;
    helpers::assert_close(pcr_rr_analytic, expected_pcr_rr, 1e-10,
...
fn validation_elastica_post_buckling() {
    ...
    helpers::assert_close(k0_k, PI / 2.0, 0.001, "K(0) = pi/2");
    helpers::assert_close(k0_e, PI / 2.0, 0.001, "E(0) = pi/2");

    // At k = 0 (no deformation): P/Pcr = 1, delta = 0
    let p_ratio_0 = (k0_k / (PI / 2.0)).powi(2);
    let delta_ratio_0 = 1.0 - k0_e / k0_k;
    helpers::assert_close(p_ratio_0, 1.0, 0.001, "P/Pcr at k=0");
    helpers::assert_close(delta_ratio_0, 0.0, 0.001, "delta/L at k=0");
...
fn validation_plate_shear_buckling() {
    ...
    for &(phi, expected_ks) in &test_cases {
        let ks = k_s(phi);
        helpers::assert_close(ks, expected_ks, 0.001,
            &format!("k_s at a/b={:.1}", phi));
    }

    // Verify k_s approaches 5.34 for long plates
    let ks_inf = k_s(100.0);
    helpers::assert_close(ks_inf, 5.34, 0.01, "k_s for very long plate");

    // Square plate: k_s = 9.34
    let ks_square = k_s(1.0);
    helpers::assert_close(ks_square, 9.34, 1e-10, "k_s for square plate");
    ...
    helpers::assert_close(tau_cr_square, expected_tau, 1e-10, "tau_cr square plate");
    ...
    helpers::assert_close(ratio, 9.34 / 4.0, 1e-10,
...
fn validation_double_modulus_reduced_modulus_theory() {
    ...
    for &et in &et_values {
        ...
        if (et - e).abs() < 1.0 {
            helpers::assert_close(er, e, 0.001, "Er = E when Et = E");
        }
    ...
    helpers::assert_close(er, expected_er, 1e-10, "Er at Et=80000");
    ...
    helpers::assert_close(er_elastic, e, 1e-10, "Er = E when Et = E");
    ...
    helpers::assert_close(er_half, expected_er_half, 0.001, "Er at Et = E/2");
