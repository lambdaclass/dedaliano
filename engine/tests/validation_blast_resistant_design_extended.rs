...
fn blast_clearing_effect() {
    ...
    helpers::assert_close(s, 3.0, 0.001, "Clearing distance S");
...
fn blast_negative_phase() {
    ...
    helpers::assert_close(i_ratio, p_neg_ratio * (td_neg / td_pos), 0.01, "Impulse ratio");
    ...
    helpers::assert_close(td_total, 52.5, 0.01, "Total waveform duration ms");
}
...
fn blast_equivalent_static_beam() {
    ...
    let n_elem = 8;
    let input = helpers::make_ss_beam_udl(n_elem, l, e, a, iz, -q_blast);
    ...
    helpers::assert_close(mid_disp.uy.abs(), delta_st, 0.02, "Midspan deflection vs analytical");
...
fn blast_portal_frame_sway() {
    ...
    helpers::assert_close(sum_rx.abs(), f_horizontal, 0.05, "Horizontal reaction equilibrium");
...
fn blast_energy_absorption_steel() {
    ...
    let q_u: f64 = ru / l;
    let input = helpers::make_ss_beam_udl(8, l, e_val, a_sec, iz, -q_u);
    let result = dedaliano_engine::solver::linear::solve_2d(&input).unwrap();
    ...
    helpers::assert_close(mid_disp.uy.abs(), x_el, 0.03, "Elastic deflection at Ru");
...
fn blast_progressive_collapse_catenary() {
    ...
    let input = helpers::make_ss_beam_udl(12, l_new, e, a_sec, iz, -w_total);
    let result = dedaliano_engine::solver::linear::solve_2d(&input).unwrap();
    ...
    helpers::assert_close(mid_disp.uy.abs(), delta_analytical, 0.02, "Bridging span deflection");
